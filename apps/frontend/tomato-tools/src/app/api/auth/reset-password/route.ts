...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validationResult.success) {
      logger.warn({ errors: validationResult.error.issues }, 'Invalid reset password data');
    ...
    if (result.error) {
      logger.error({ error: result.error, email }, 'Password reset email failed');
      return NextResponse.json(
        { error: result.error },
        { status: 400 }
      );
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (!validationResult.success) {
      logger.warn({ errors: validationResult.error.issues }, 'Invalid update password data');
      return NextResponse.json(
        { 
          error: 'Invalid input data',
          details: validationResult.error.issues 
