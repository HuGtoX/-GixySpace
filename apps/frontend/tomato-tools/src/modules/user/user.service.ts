...
export class UserService {
  ...
  async createUserProfile(profileData: NewUserProfile): Promise<UserProfile> {
    ...
    try {
      ...
      this.logger.info({ userId: profileData.userId }, 'User profile created successfully');
      return profile;
    } catch (error) {
      this.logger.error({ error, userId: profileData.userId }, 'Failed to create user profile');
      throw error;
    }
  }
  ...
  async createUserSession(sessionData: NewUserSession): Promise<UserSession> {
    ...
    try {
      ...
      this.logger.info({ userId: sessionData.userId, sessionId: session.id }, 'User session created');
      return session;
    } catch (error) {
      this.logger.error({ error, userId: sessionData.userId }, 'Failed to create user session');
      throw error;
    }
  }
  ...
  async createPasswordResetToken(userId: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error({ error, userId }, 'Failed to create password reset token');
      throw error;
    }
  }
  ...
  async validatePasswordResetToken(token: string): Promise<string | null> {
    try {
      ...
      if (!resetToken || resetToken.expiresAt < new Date()) {
        this.logger.warn({ token: token.substring(0, 8) + '...' }, 'Invalid or expired reset token');
        return null;
      }
  ...
  async markPasswordResetTokenAsUsed(token: string): Promise<void> {
    try {
      ...
      this.logger.error({ error }, 'Failed to mark password reset token as used');
      throw error;
    }
  }
}