...
export async function updateSession(request: NextRequest) {
  ...
  requestLogger.debug("Supabase Middleware Module is Processing request");
  ...
  try {
    ...
    if (error) {
      requestLogger.error(
        { error: error.message },
        "Auth error in Supabase Middleware Module",
      );
    } else if (user) {
    ...
    requestLogger.error(
      { error },
      "Failed to get user in in Supabase Middleware Module",
    );
  }
  ...
  requestLogger.debug(
    "Supabase Middleware Module: Request processed successfully",
  );
